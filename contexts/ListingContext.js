...
export const ListingProvider = ({ type, ssrProps, cr, children }) => {
  ...
  const [listingBio, setListingBio] = useState(
    type === "flatmate" ? ssrProps.listingBio : null
  );
  const [listingFlatBio, setListingFlatBio] = useState(
    type === "flat" ? ssrProps.listingFlatBio : null
  );
  const [listingPersonBio, setListingPersonBio] = useState(
    type === "flat" ? ssrProps.listingPersonBio : null
  );
  const [listingPersonBoxes, setListingPersonBoxes] = useState(
    JSON.parse(ssrProps.listingPersonBoxes)
  );
  const [listingPersonTags, setListingPersonTags] = useState(
    JSON.parse(ssrProps.listingPersonTags)
  );
  const [listingFlatBoxes, setListingFlatBoxes] = useState(
    type === "flat" ? JSON.parse(ssrProps.listingFlatBoxes) : null
  );
  const [listingFlatTags, setListingFlatTags] = useState(
    type === "flatmate" ? JSON.parse(ssrProps.listingFlatTags) : null
  );
  const [listingPremium, setListingPremium] = useState(ssrProps.listingPremium);
  const [listingFans, setListingFans] = useState(
    ssrProps.listingFans ? ssrProps.listingFans : []
  );
  ...
  const listingLoad = () => {
    ...
    if (ssrProps.status != "success" && ssrProps.status != "client-side") {
      snackBar(
        "Uživatel neexistuje, nebo je jeho inzerát nedokončený.",
        "error"
      );
      return;
    }
    ...
    if (
      currentUser &&
      (ssrProps.listingFriends.includes(currentUser.uid) ||
        currentUser.uid === ssrProps.uid)
    ) {
      getListingContact(id)
        .then((docs) => {
  ...
  const handleSave = () => {
    ...
    if (!budget || budget < 1000 || budget > 60000) {
      snackBar(
        `Prosím zadejte správnou hodnotu do ${
          listingInfo.type === "flatmate" ? "rozpočtu." : "nájemného."
        }`
      );
      setLoading(false);
      window.scrollTo(0, 0);
    ...
    updateListing(JSON.stringify(updateListingInfo))
      .then((response) => {
        if (addedPfp) {
  ...
  const handleImgDelete = (type) => {
    if (type === "pfp") {
      ...
      deleteImgs(JSON.stringify(imageInfo))
        .then((response) => {
          setPfp(null);
          setLoading(false);
        })
  ...
  const loadClientSide = () => {
    getListing(id)
      .then((doc) => {
        ...
        setListingName(
          type === "flatmate"
            ? docData.userInfo.username
            : `${docData.flatBoxes.layout}${
                docData.flatBoxes.layout ? " " : ""
              }${docData.flatBoxes.location}`
        );
