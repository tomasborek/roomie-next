      ...
      <div className="brand">
        <div className="brand-logo">
          <img
            src={
              variant === "gradient"
                ? "/img/logos/logo-white-small.png"
                : "/img/logos/logo-small.png"
            }
            alt=""
          />
        </div>
        ...
        currentUser ? (
          <div className="navbar navbar-logged">
            {notifications?.length ? (
              ...
              >
                <i
                  ...
                  className={`fa${
                    notificationDropdown ? "s" : "r"
                  } fa-bell navbar-notifications`}
                ></i>
              </Badge>
            ) : (
              !notifications?.length && (
            ...
            {currentUserInfo?.mainInfo.pfp ? (
              ...
              <div onClick={handleMyListing} className="navbar-profile"></div>
            )}
            {currentUserInfo && (
              <p onClick={handleMyListing} className="navbar-name">
                {currentUserInfo.mainInfo.username}
              </p>
            )}

            <motion.i
              onClick={() => setIsDropdownActive((prevState) => !prevState)}
              animate={isDropdownActive ? { rotate: -180 } : ""}
              initial={{ rotate: 0 }}
            ...
            >
              <ul className="dropdown-list">
                <li onClick={handleMyListing} className="list-item">
                  <i className="fas fa-home item-icon"></i> Můj inzerát
                </li>
                <li
                  onClick={() => router.push("/friends")}
                  className="list-item"
                >
                  <i className="fas fa-users item-icon"></i> Přátelé
                </li>
                <li
                  onClick={() => router.push("/requests/recieved")}
                  className="list-item"
                >
                  <i className="fas fa-envelope item-icon"></i> Žádosti
                </li>
                <li onClick={() => router.push("/liked")}>
                  <i className="fas fa-heart"></i> Oblíbené
                </li>
                <li
                ...
                >
                  {" "}
                  <i className="fas fa-pen item-icon"></i> Účet a soukromí
                </li>
                <li
                  ...
                  style={{ color: "red" }}
                >
                  <i className="fas fa-sign-out-alt item-icon"></i> Odhlásit
                </li>
              </ul>
            </Dropdown>
            <Dropdown
                    ...
                    {Object.keys(notifications).length > 0 ? (
                      Object.keys(notifications).map((not, id) => (
                        <li
                          key={id}
                          onClick={() =>
                            notifications[not].type === "recievedRequest"
